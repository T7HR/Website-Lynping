...
export default async function OwnerStaffPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="จัดการพนักงานประมูล ( Auction Staff )"
        description=""
      />
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        ...
        <div className="space-y-4">
          {pendingCommands.length > 0 && (
            <div className="card divide-y divide-white/10 overflow-hidden">
              ...
              {pendingCommands.map(cmd => (
                <div key={cmd.id} className="flex items-center justify-between gap-4 p-5 text-sm">
                  <div>
                    <p className="font-bold">{cmd.action === "remove_staff" ? "ลบ" : "เพิ่ม หรือ แก้ไข"} <span className="text-zinc-300">{cmd.staff_id}</span></p>
                    <p className="text-zinc-400">waiting category: {cmd.waiting_category_id || "ใช้ค่าเดิม/ค่า default"}</p>
                  </div>
                  <span className="rounded-full border border-yellow-300/20 bg-yellow-400/10 px-3 py-1 text-xs font-bold text-yellow-100">กำลังดำเนินการ</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div className="card divide-y divide-white/10 overflow-hidden">
            <div className="p-5">
              <p className="panel-title">Server Sync Website To Bot</p>
              <h2 className="mt-1 text-lg font-black">รายชื่อพนักงานประมูลในระบบ</h2>
            </div>
            {staff.map(s => {
