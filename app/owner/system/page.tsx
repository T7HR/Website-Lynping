  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card overflow-hidden">
        {rows.map((row: any) => (
          <div key={row.key} className="flex items-center justify-between gap-4 border-b border-white/10 p-5 hover:bg-white/[0.03]">
            <div>
              <p className="panel-title">Mirror Row</p>
              <b className="mt-1 block">{row.key}</b>
              <p className="text-sm text-zinc-400">{row.file_name}</p>
            </div>
            <span className="rounded-md bg-white/[0.04] px-3 py-2 text-xs text-zinc-500">{row.updated_at}</span>
          </div>
        ))}
